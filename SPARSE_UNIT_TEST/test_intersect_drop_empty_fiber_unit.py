...
def init_module():
    ...
    verilog(dut, filename=f"./modules/Intersect.sv",
            optimize_if=False)
    sparse_helper.update_tcl("intersect_drop_empty_fiber_tb")
...
def create_gold(in_crd1, in_crd2, in_ref1, in_ref2):
    ...
    inter = Intersect2()
    stkndrop_crd = EmptyFiberStknDrop()
    stkndrop_ref1 = EmptyFiberStknDrop()
    stkndrop_ref2 = EmptyFiberStknDrop()
    out_crd = []
    out_ref1 = []
    out_ref2 = []
    ...
    while not done and time < TIMEOUT:
        ...
        stkndrop_crd.set_in_stream(inter.out_crd())
        stkndrop_ref1.set_in_stream(inter.out_ref1())
        stkndrop_ref2.set_in_stream(inter.out_ref2())

        inter.update()
        stkndrop_crd.update()
        stkndrop_ref1.update()
        stkndrop_ref2.update()

        out_crd.append(stkndrop_crd.out_val())
        out_ref1.append(stkndrop_ref1.out_val())
        out_ref2.append(stkndrop_ref2.out_val())

        # print("Timestep", time, "\t Crd:", inter.out_crd(), "\t Ref1:", inter.out_ref1(), "\t Ref2:", inter.out_ref2())

        done = inter.done and stkndrop_crd.out_done() and stkndrop_ref1.out_done() and stkndrop_ref2.out_done()
        time += 1
...
def module_iter_basic(test_name, add_test=""):
    ...
    if add_test == "":
        sim_result = subprocess.run(["make", "sim", "TEST_TAR=intersect_drop_empty_fiber_tb.sv", "TOP=intersect_drop_empty_fiber_tb",\
                             "TEST_UNIT=Intersect.sv"], capture_output=True, text=True)
    else:
        sim_result = subprocess.run(["make", "sim", "TEST_TAR=intersect_drop_empty_fiber_tb.sv",\
                             "TOP=intersect_drop_empty_fiber_tb", "TX_NUM_GLB=2", "TEST_UNIT=Intersect.sv"\
