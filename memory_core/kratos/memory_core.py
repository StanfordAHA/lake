...
class MemoryCore(Generator):
    def __init__(self,
        ...
        # Linebuffer Signals
        self._lb_addr = self.var("lb_addr",
                                 self.addr_width_macro,
                                 size=self.banks,
                                 explicit_array=True,
                                 packed=True)
        self._lb_mem_data_out = self.var("lb_mem_data_out",
                                         self.mem_width,
                                         size=self.banks,
                                         explicit_array=True,
                                         packed=True)
        ...
        # Fifo Signals
        self._fifo_addr = self.var("fifo_addr",
                                   self.addr_width_macro,
                                   size=self.banks,
                                   explicit_array=True,
                                   packed=True)
        self._fifo_mem_data_out = self.var("fifo_mem_data_out",
                                           self.mem_width,
                                           size=self.banks,
                                           explicit_array=True,
                                           packed=True)
        ...
        # SRAM Signals
        self._sram_addr = self.var("sram_addr",
                                   self.addr_width_macro,
                                   size=self.banks,
                                   explicit_array=True,
                                   packed=True)
        self._sram_mem_data_out = self.var("sram_mem_data_out",
                                           self.mem_width,
                                           size=self.banks,
                                           explicit_array=True,
                                           packed=True)
        ...
        # UB Signals
        self._db_addr = self.var("db_addr",
                                 self.addr_width_macro,
                                 size=self.banks,
                                 explicit_array=True,
                                 packed=True)
        self._db_mem_data_out = self.var("db_mem_data_out",
                                         self.mem_width,
                                         size=self.banks,
                                         explicit_array=True,
                                         packed=True)
        ...
        self._mem_data_in = self.var("mem_data_in",
                                     ...
                                     explicit_array=True,
                                     packed=True)
        self._mem_addr = self.var("mem_addr",
                                  self.addr_width_macro,
                                  size=self.banks,
                                  explicit_array=True,
                                  packed=True)
        ...
        # Various signals
        self._stride = self.var("stride",
                                16,
                                size=self.iterator_support,
                                explicit_array=True,
                                packed=True)
        self._range = self.var("range",
                               32,
                               size=self.iterator_support,
                               explicit_array=True,
                               packed=True)
        ...
        self.wire(self._read_config_data,
                  zext(self._mem_data_out[self._sram_sel], self._read_config_data.width))

        # Chaining logic
        self.wire(self._data_in_int,
                  ternary(self._enable_chain,
                          self._chain_in,
                          self._data_in))
        self.wire(self._wen_in_int,
                  ternary(self._enable_chain,
                          self._chain_wen_in,
                          self._wen_in))
        self.wire(self._chain_out,
                  ternary(self._enable_chain & self._chain_wen_in,
                          self._chain_in,
                          self._data_out))
        self.wire(self._chain_valid_out,
                  (self._enable_chain & self._chain_wen_in) | self._valid_out)

        self.sram_reads_per_bank = self.mem_depth / 256

        for i in range(self.banks):
            self.wire(self._mem_cen[i],
                      (self._mem_cen_int[i] &
                       (self._mem_wen[i] | self._mem_ren[i]) &
                       (self._clk_en | self._config_en_sram.r_or())))
    ...
    def mux_sram_signals(self):
        if self._config_en_sram.r_or():
            ...
            self._mem_wen[0] = (self._config_write &
                                (self._config_en_sram[0] | self._config_en_sram[1]))
            self._mem_wen[1] = (self._config_write &
                                (self._config_en_sram[2] | self._config_en_sram[3]))
            self._mem_ren[0] = (self._config_read &
                                (self._config_en_sram[0] | self._config_en_sram[1]))
            self._mem_ren[1] = (self._config_read &
                                (self._config_en_sram[2] | self._config_en_sram[3]))
            self._mem_addr[0] = concat(self._config_en_sram[3] | self._config_en_sram[1],
                                       self._config_addr[31, 24])
            self._mem_addr[1] = concat(self._config_en_sram[3] | self._config_en_sram[1],
                                       self._config_addr[31, 24])
            ...
            self._data_out = ternary(self._enable_chain & self._chain_wen_in,
                                     self._chain_in,
                                     self._db_out)
    ...
    def instantiate_fifo(self):
        self.add_child("fifo_ctrl",
                       FIFOControl(self.data_width,
                                   self.banks,
                                   self.mem_depth),
        ...
        self.wire(self["fifo_ctrl"].ports.ren,
                  ternary(self._mode == 1, self._ren_in, self._ren_lb_to_fifo))
    ...
    def instantiate_sram(self):
        self.add_child("sram_ctrl",
                       SRAMControl(self.data_width,
                                   self.banks,
                                   self.mem_depth),
    ...
    def instantiate_db(self):
        self.add_child("db_ctrl",
                       DoubleBufferControl(self.data_width,
                                           self.mem_depth,
                                           self.banks,
                                           self.iterator_support),
                       comment="===== DOUBLEBUFFER =====//")
    ...
    def instantiate_memory(self):
        for i in range(self.banks):
            self.add_child(f"mem_inst_{i}",
                           SRAMStub(self.data_width,
                                    self.mem_depth),
                           comment=f"===== SRAM MACRO {i} =====//")
            ...
            self.wire(self[f"mem_inst_{i}"].ports.i_wen,
                      self._mem_wen[i] & (self._clk_en | self._config_en_sram.r_or()))
