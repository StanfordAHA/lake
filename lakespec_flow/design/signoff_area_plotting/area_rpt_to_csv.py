...
def process_area_report(input_file_path, output_csv_path):
    ...
    with open(output_csv_path, mode='w', newline='') as csv_file:
        ...
        for line in area_report_lines[2:]:
            if line.strip():  # Skip any potentially empty lines
                ...
                if "ag" in row_data[0] and "AddressGenerator" in row_data[1] and "/" not in row_data[0]:
                    try:
                        total_area_ag += float(row_data[3])
                    except ValueError:
                        pass  # Ignore rows where conversion to float fails
                if "sg" in row_data[0] and "ScheduleGenerator" in row_data[1] and "/" not in row_data[0]:
                    try:
                        total_area_sg += float(row_data[3])
                    except ValueError:
                        pass
                if "id" in row_data[0] and "IterationDomain" in row_data[1] and "/" not in row_data[0]:
                    try:
                        total_area_id += float(row_data[3])
                    except ValueError:
                        pass
                ######## clock network
                if "ag" in row_data[0] and "AddressGenerator" in row_data[1] and "clk" in row_data[0]:
                    try:
                        total_area_ag -= float(row_data[3])
                    except ValueError:
                        pass  # Ignore rows where conversion to float fails
                if "sg" in row_data[0] and "ScheduleGenerator" in row_data[1] and "clk" in row_data[0]:
                    try:
                        total_area_sg -= float(row_data[3])
                    except ValueError:
                        pass
                if "id" in row_data[0] and "IterationDomain" in row_data[1] and "clk" in row_data[0]:
                    try:
                        total_area_id -= float(row_data[3])
                    except ValueError:
                        pass
    ...
    # Data for plotting
    labels = ['Address\nGenerator', 'Schedule\nGenerator', 'Iteration\nDomain']
    sizes = [total_area_ag, total_area_sg, total_area_id]  # example data values
    colors = ['#003366','#0073E6','#ADD8E6']
    ...
    plt.figure(figsize=(11, 11))  # Adjusting figure size to prevent cropping
    ...
    # Data for plotting
    labels_totals = ['Storage', 'Control', 'Clock Gating']
    ...
    plt.figure(figsize=(12, 9))  # Adjusting figure size to prevent cropping
    ...
    print(f"Total accumulated area clock gating: {clock_area}\n")
