...
`endif
    ...
    */
    ...
    // VCD dumping for power estimation. Off by default — enable with +dump_vcd=1.
    // The vcd2saif step downstream consumes outputs/run.vcd.
    integer dump_vcd_enable = 0;
    initial begin
        if ($value$plusargs("dump_vcd=%d", dump_vcd_enable) && dump_vcd_enable == 1) begin
            $display("VCD dumping enabled -> waveforms.vcd");
