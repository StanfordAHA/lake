...
class SinglePortMemory(Generator):
    ...
    def instantiate_single_port(self, banks_tall, banks_wide, macro_width, macro_depth):
        '''
            ...
            for j in range(banks_wide):
                ...
                if(banks_tall > 1):
                    self.wire(self[f"sram_d{i}_w{j}"].ports.i_wen,
                            self._wen & (self._addr[self.full_addr - 1, self.addr_width_macro] == i)
                            )
                    self.wire(self[f"sram_d{i}_w{j}"].ports.i_cen,
                            (self._ren | self._wen) & (self._addr[self.full_addr - 1, self.addr_width_macro] == i)
                            )
                else:
