...
class ScannerPipe(MemoryController):
    def __init__(self,
        ...
        self._glb_addr_base = self.input("glb_addr_base", 16)  # Exact size to be determined
        self._glb_addr_base.add_attribute(ConfigRegAttr("Base address for global memory"))

        self._glb_addr_stride = self.input("glb_addr_stride", 16)
        self._glb_addr_stride.add_attribute(ConfigRegAttr("Stride for global memory"))
...
# =============================
        ...
        if self.split_mem_requests:
            ...
            # Need to wire up the requests
            # self.wire(self._seg_grant_push, self._seg_req_push & self._no_outfifo_full[0])
            # self.wire(self._crd_grant_push, self._crd_req_push & self._no_outfifo_full[1])
            ...
            # self.wire(self._addr_out_to_fifo[0], self._seg_addr_out_to_fifo)
            # self.wire(self._addr_out_to_fifo[1], self._crd_addr_out_to_fifo)
            # self.wire(self._op_out_to_fifo[0], self._seg_op_out_to_fifo)
            # self.wire(self._op_out_to_fifo[1], self._crd_op_out_to_fifo)

            # self.wire(self._addr_out_fifo_push[0], self._seg_grant_push)
            # self.wire(self._addr_out_fifo_push[1], self._crd_grant_push)
            # self.wire(self._op_out_fifo_push[0], self._seg_grant_push)
            # self.wire(self._op_out_fifo_push[1], self._crd_grant_push)

            # self.wire(self._ID_out_fifo_push[0], self._seg_grant_push)
            # self.wire(self._ID_out_fifo_push[1], self._crd_grant_push)
            # self.wire(self._ID_out_to_fifo[0], self._seg_ID_out_to_fifo)
            # self.wire(self._ID_out_to_fifo[1], self._crd_ID_out_to_fifo)
...
# =================================
        ...
        if num_ports == 2:
            push_reserve_crd_res_fifo = self._rd_rsp_fifo_valid[crd_port_num] | (self._rd_rsp_fifo_valid[0] & ((self._block_mode & ~self._push_glb_addr) | (self._lookup_mode & ~self._block_mode)))  # TODO: crd res push
        else:
        ...
        self.add_child("crd_res_fifo",
                       ...
                       rst_n=self._rst_n,
                       clk_en=self._clk_en,
                    #    data_in_0=kts.concat(kts.const(0, 1), self._rd_rsp_fifo_out_data[crd_port_num][self.data_width - 1, 0]),
                    ...
                    #    push_reserve=self._rd_rsp_fifo_valid[crd_port_num] & ((self._rd_rsp_fifo_out_data[crd_port_num][self.data_width] == kts.const(1, 1)) | self._block_mode | self._lookup_mode),
...
# =============================
        ...
        # self.wire(self._ptr_in, self._rd_rsp_fifo_out_data[seg_port_num][self.data_width - 1, 0])
        if num_ports == 2:
            p2 = 1
        else:
            p2 = 0
...
# do dense streaming, and do the block reads
        ...
        BLOCK_1_SIZE_PUSH = self.scan_fsm_crd.add_state("BLOCK_1_SIZE_PUSH")
        ...
        self.scan_fsm_crd.output(self._push_glb_addr, default=kts.const(0, 1))
        ...
        BLOCK_1_SIZE_REC.output(self._push_glb_addr, self._rd_rsp_fifo_valid[0] & ~self._block_rd_fifo.ports.full)

        ######################
        # BLOCK_1_SIZE_PUSH
        ######################
        BLOCK_1_SIZE_PUSH.output(self._crd_addr_out_to_fifo, 0)
        ...
        BLOCK_1_SIZE_PUSH.output(self._pos_out_to_fifo, 0)
        BLOCK_1_SIZE_PUSH.output(self._crd_out_to_fifo, 0)
        BLOCK_1_SIZE_PUSH.output(self._inc_req_made_crd, 0)
        BLOCK_1_SIZE_PUSH.output(self._clr_req_made_crd, 1)
        BLOCK_1_SIZE_PUSH.output(self._inc_req_rec_crd, 0)
        BLOCK_1_SIZE_PUSH.output(self._clr_req_rec_crd, 0)
        BLOCK_1_SIZE_PUSH.output(self._crd_res_fifo_push_alloc, 0)
        BLOCK_1_SIZE_PUSH.output(self._crd_res_fifo_push_fill, 0)
        BLOCK_1_SIZE_PUSH.output(self._ptr_reg_en, self._rd_rsp_fifo_valid[0])
        BLOCK_1_SIZE_PUSH.output(self._seg_res_fifo_pop, 0)
...
# ===================================
        ...
        #                                                                                   ~self._coord_fifo.ports.full)),
        #                                                           ~self._coord_fifo.ports.full)))
        self.wire(self._crd_res_fifo_pop, kts.ternary(self._vector_reduce_mode & self._output_row_fully_accumulated,  # TODO: pop res fifo
