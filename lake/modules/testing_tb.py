...
#        self.out_row_index = self.var("out_row_index", clog2(2*stencil_height))
        self.count = self.var("count", clog2(mem_word_width) + 1)
        # sequential blocks
...
#            seq.add(if_num)
#            if_ = IfStmt((self.valid_input[i] == 1) & (self.first == 0))
#            if__ = IfStmt((self.valid_input[i] == 1) & (self.first == 1))
#            if__.then_(self.count.assign(self.count + 1))
#            comb.add_stmt(if__)
#            if_.then_(self.first.assign(1))
#            if_.then_(self.valid_data[(test)[clog2(self.mem_word_width) - 1, 0]].assign(self.mem_data[i]))
#            test2 = num_valid_ - 1
#            test = test2[clog2(self.mem_word_width) - 1, 0]
