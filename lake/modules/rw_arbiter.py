...
class RWArbiter(Generator):
    ...
    def __init__(self,
        ...
        self._data_to_mem = self.input("data_to_mem", self.fetch_width)
        ...
        self._next_rd_port = self.var("rd_port", self.int_out_ports)
    ...
    def mem_controls(self):
        ...
        self._cen_mem = self._wen_in | (kts.reduce_or(self._ren_in))
    ...
    def set_next_read_port(self):
        ...
        for i in range(self.int_out_ports):
            if(self._next_rd_port == 0):
                self._next_rd_port = self._ren_in[i]
    ...
    def next_read_valid(self):
        if ~self._rst_n:
            ...
            self._rd_valid = ~self._wen_in & kts.reduce_or(self._ren_in)
