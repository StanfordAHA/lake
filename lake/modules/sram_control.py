...
class SRAMControl(Generator):
    def __init__(self, data_width, banks, mem_depth):
        ...
        self._sram_to_mem_data = self.output("sram_to_mem_data", self.data_width, size=self.banks, explicit_array=True, packed=True)
        self._sram_to_mem_cen = self.output("sram_to_mem_cen", self.banks)
        self._sram_to_mem_wen = self.output("sram_to_mem_wen", self.banks)
        self._sram_to_mem_addr = self.output("sram_to_mem_addr", self.mem_addr_width, size=self.banks, explicit_array=True, packed=True)
        self._mem_to_sram_data = self.input("mem_to_sram_data", self.data_width, size=self.banks, explicit_array=True, packed=True)
        ##### PORT DEFS: end
        ...
        for i in range(self.banks):
            self.wire(self._bank_seld[i], self._addr[self.full_addr - 1, self.full_addr - self.bank_addr_width] == i)
