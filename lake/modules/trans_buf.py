...
class TransposeBuffer(Generator):
    def __init__(self, word_width, mem_word_width, range_, stride, stencil_height):
        ...
        self.read_row_ready = self.var("read_row_ready", 1)
    ...
    def update_index_vars(self):
        if (self.rst == 0):
            ...
            self.read_row_ready = 1
        elif (((stencil_height >= mem_word_width) & (self.read_row == (stencil_height - 1))) \
                | ((stencil_height < mem_word_width) & (self.out_col == mem_word_width))):
            self.read_row = 0
            self.out_col = 0
    ...
    def update_read_buf(self):
        if (self.read_row_ready == 1):
            for i in range(mem_word_width):
                self.tb[self.read_row + self.read_buf * stencil_height] = self.sram_input[i]
