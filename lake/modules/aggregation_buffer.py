...
class AggregationBuffer(Generator):
    ...
    def __init__(self,
        ...
        for i in range(self.agg_height):
            ...
            self.add_child(f"agg_{i}", Aggregator(self.data_width, mem_word_width=self.mem_width))
            # Wire it up
            ...
            self.wire(self[f"agg_{i}"].ports._valid_in, self._valid_demux[i])
            self.wire(self[f"agg_{i}"].ports.agg_out, self._aggs_out[i])
