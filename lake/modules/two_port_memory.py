...
class TwoPortMemory(Generator):
    def __init__(self, 
        ...
        print(f"Generating memory with {banks_wide} banks for width and {banks_tall} banks for capcity")

        ##### LOCAL SIGNALS: begin
        # This is the output of all banks (tallwise)
        self._memory_space = self.var("memory_space",  width=desired_width, size=banks_tall, packed=True)
    ...
    def instantiate_true_two_port(self, banks_tall, banks_wide, macro_width, macro_depth):
        ...
        for i in range(banks_tall):
            if(banks_wide > 1):
                self.wire(self._memory_space[i], concat(*[self[f"sram_d{i}_w{j}"].ports.o_data for j in range(banks_wide)]))
            else:
