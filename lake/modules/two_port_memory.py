...
class TwoPortMemory(Generator):
    def __init__(self, 
        ...
        print(f"Generating memory with {banks_wide} banks for width and {banks_tall} banks for capacity")

        ##### LOCAL SIGNALS: begin
        # This is the output of all banks (tallwise)
        self._memory_space = self.var("memory_space",  width=desired_width, size=banks_tall, packed=True, explicit_array=True)
        ...
        self._bank_sel_reg = self.var("bank_sel_reg", max(self.full_addr - self.addr_width_macro, 1))
        ##### LOCAL_SIGNALS: end
            ...
            for j in range(banks_wide):
                ...
                if(banks_tall > 1):
                    self.wire(self[f"sram_d{i}_w{j}"].ports.i_wen,
                            self._wen & (self._wr_addr[self.full_addr - 1, self.addr_width_macro] == i)
                            )
                    self.wire(self[f"sram_d{i}_w{j}"].ports.i_ren,
                            self._ren & (self._rd_addr[self.full_addr - 1, self.addr_width_macro] == i)
                            )
                else:
                ...
                self.wire(self[f"sram_d{i}_w{j}"].ports.i_data, self._wr_data[((j+1)*macro_width)-1, j*macro_width])

        for i in range(banks_tall):
            if(banks_wide > 1):
                self.wire(self._memory_space[i], concat(*[self[f"sram_d{i}_w{j}"].ports.o_data for j in range(banks_wide-1, -1, -1)]))
            else:
    ...
    def build_two_port(self, banks_tall, banks_wide, macro_width, macro_depth):
        '''
        Takes a single port memory and constructs a virtualized two port memory from a single port memory
        '''
...
if __name__ == "__main__":
    #tpm_dut = TwoPortMemory(macro_depth=128, macro_width=16, desired_depth=128, desired_width=16, two_port=1)
    tpm_dut = TwoPortMemory(macro_depth=128, macro_width=16, desired_depth=256, desired_width=32, two_port=1)
