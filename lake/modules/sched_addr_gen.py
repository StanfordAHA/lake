...
class SchedAddrGen(Generator):
    ...
    def __init__(self,
        ...
        super().__init__(f"sched_addr_gen_{iterator_support}_{config_width}", debug=True)
        ...
        # self._after_start = self.output("after_start", 1)
        ...
        # self.wire(self._after_start, self._strt_addr <= self._cycle_count)
    ...
    def calculate_address(self):
        ...
        # elif self._after_start:
            ...
            if self._restart:
                self._current_addr = self._strt_addr - 1
            elif self._current_addr == 0:
                self._current_addr = self._strides[self._mux_sel] - 1
            else:
...
if __name__ == "__main__":
    db_dut = SchedAddrGen(iterator_support=6)
    verilog(db_dut, filename="sched_addr_gen.sv")
