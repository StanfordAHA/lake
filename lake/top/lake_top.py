...
class LakeTop(Generator):
    def __init__(
        ...
        #self.input_port_sched_width = input_port_sched_width
        ...
        for i in range(self.interconnect_input_ports):
            self._i_strides.append(self.input(f"stride_i_{i}", 32, size=self.input_iterator_support, packed=True, explicit_array=True))
            self._i_ranges.append(self.input(f"range_i_{i}", 32, size=self.input_iterator_support, packed=True, explicit_array=True))
        ...
        for i in range(self.banks):
            self._i_port_scheds.append(self.input(f"i_port_sched_b_{i}", self.input_port_sched_width, size=self.input_max_port_sched, packed=True, explicit_array=True))
        self._i_port_periods = self.input("i_port_periods", clog2(self.input_max_port_sched), size=self.banks)
       ...
       # self._valid_in = self.input("valid_in", 1, size=self.interconnect_input_ports, explicit_array=True, packed=True)
        ...
        for i in range(self.interconnect_output_ports):
            self._o_strides.append(self.input(f"stride_o_{i}", 32, size=self.iterator_support, packed=True, explicit_array=True))
            self._o_ranges.append(self.input(f"range_o_{i}", 32, size=self.iterator_support, packed=True, explicit_array=True))
        ...
        for i in range(self.interconnect_input_ports):
            ...
            self.wire(self[f"input_addr_ctrl"].ports.valid_in[i], self[f"agg_in_{i}"].ports._valid_out) # Also demux these

        # Connect these inputs ports to an address generator'
        self.add_child(f"input_addr_ctrl", InputAddrCtrl(self.interconnect_input_ports, self.mem_depth, banks=self.banks,
                                                            iterator_support=self.iterator_support, max_port_schedule=64,
        ...
        for i in range(self.interconnect_input_ports):
            ...
            self.wire(self[f"input_addr_ctrl"].ports[f"starting_addr_p_{i}"], self._i_starting_addrs[i])
            self.wire(self[f"input_addr_ctrl"].ports[f"dimensionality_{i}"], self._i_dimensionalities[i])
