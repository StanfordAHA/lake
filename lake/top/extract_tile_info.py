...
def extract_top_config(circuit_gen: kts.Generator):
    ...
    for port_name in int_gen.get_port_names():
        ...
        for i in range(len(curr_port.size)):
            curr_port.size[i] -= 1
        # print(f"{curr_port.size}[{curr_port.width}] {port_name}: " +
            #   f"{curr_port.explicit_array}")
    return config_list
...
def get_interface(circuit_gen: kts.Generator):
    ...
    for port_name in int_gen.get_port_names():
        ...
        cr_attr = attrs[0]
        # Now we have this
        intf_sigs.append((port_name, curr_port.size, curr_port.width, cr_attr.get_control(), str(curr_port.port_direction), curr_port.explicit_array))
        # print(f"{curr_port.size}[{curr_port.width}] {port_name}: " +
            #   f"  is_cont {cr_attr.get_control()}, in_out: {curr_port.port_direction}, explicit_array: {curr_port.explicit_array}")
    return intf_sigs
