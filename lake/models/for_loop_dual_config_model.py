...
class ForLoopDualConfigModel(Model):
    ...
    def get_dim_cnt2(self):
        return self.dim_cnt2
    ...
    def step(self):
        if (self.config_sel == 0) and (self.config["dimensionality"] != 0):
            # after restart, needs to clear both dim_counters
            if self.restart:
            ...
            self.restart = 0
            for i in range(self.config["dimensionality2"]):
                if (self.dim_cnt2[i] == (self.config[f"ranges2_{i}"] + 1)):
                    self.dim_cnt2[i] = 0
                else:
                    self.dim_cnt2[i] = self.dim_cnt2[i] + 1
                    break
