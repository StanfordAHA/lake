...
TABLES = [
    ...
    Entry("lake_interfaces", "tab:lake_interfaces",
          ...
          "Lake Component interface summary (constructor signatures auto-scraped)",
          "real", g.lake_interfaces,
          notes="Skeleton: signatures from lake/spec/*.py; prose descriptions are hand-authored (marked % TODO in-file)."),
    Entry("compiler_info", "tab:compiler_info",
          "tables/compiler_info.tex", "table",
          "compiler↔component metadata handshake (structure only)",
          "real", g.compiler_info,
          notes="Skeleton: Component list scraped; per-column prose is hand-authored."),
    ...
    Entry("memtile_model_coeff", "tab:memtile_model_coeff",
          ...
          "Memory Tile regression: Lasso on logic_area (Kahng-hybrid) + power + delay",
          "real", g.memtile_model_coeff,
          notes="Power fit falls back to MissingDataError until ptpx-synth lands. "
                "Delay fit uses unsaturated points only (>99% clock ceiling dropped)."),
    Entry("memtile_model_verif", "tab:memtile_model_verif",
          "tables/memtile_model_verif.tex", "table",
          "Memtile PPA model: leave-one-experiment-out predicted vs actual",
          "real", g.memtile_model_verif,
          notes="Same fit as memtile_model_coeff; verif table is LOEO residuals."),
    Entry("exploration_applications", "tab:exploration_applications",
          "tables/exploration_applications.tex", "table",
          "app suite × memory-usage summary (rendered from THESIS/apps/registry.py)",
          "real", g.exploration_applications),
    Entry("ul_design_points", "tab:ul_design_points",
          "tables/ul_design_points.tex", "table",
          "single-level memory design points (rendered from THESIS/apps/design_points.py)",
          "real", g.ul_design_points),
    ...
    Entry("ul_ppa_summary", "tab:ul_ppa_summary",
          ...
          "PPA summary per single-level design point (area+timing live; power blank until sweep runs)",
          "real", g.ul_ppa_summary,
          notes="Power column blank until ptpx-synth flow runs; auto-fills once synth_power_w populated."),
]
